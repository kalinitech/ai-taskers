      ...
      <aside className="lg:w-64 bg-card border-r lg:min-h-screen">
        <div className="p-4 border-b">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#1E2A5E] to-[#00C2D1] flex items-center justify-center">
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                activeTab === tab.id
                  ? 'bg-gradient-to-r from-[#1E2A5E] to-[#00C2D1] text-white'
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatBox icon={Users} label="Total Taskers" value={counts.totalTaskers} color="bg-[#1E2A5E]" onClick={() => onNavigate('taskers')} />
        <StatBox icon={Building2} label="Total Employers" value={counts.totalEmployers} color="bg-[#00C2D1]" onClick={() => onNavigate('employers')} />
        <StatBox icon={CreditCard} label="Active Subscriptions" value={counts.totalSubscriptions} color="bg-[#2DD4BF]" onClick={() => onNavigate('plans')} />
        <StatBox icon={DollarSign} label="Monthly Revenue" value={`$${revenue.monthly}`} color="bg-[#2B3B7A]" onClick={() => onNavigate('analytics')} />
        <StatBox icon={ShieldCheck} label="Verified Taskers" value={counts.verifiedTaskers} color="bg-[#2DD4BF]" />
        <StatBox icon={Sparkles} label="Featured Taskers" value={counts.featuredTaskers} color="bg-[#2B3B7A]" />
        <StatBox icon={Crown} label="Premium Taskers" value={counts.premiumTaskers} color="bg-[#1E2A5E]" />
        <StatBox icon={TrendingUp} label="Active Posts" value={counts.activePosts} color="bg-[#00C2D1]" />
      </div>
          ...
          {countryDistribution.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={countryDistribution}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="count" fill="#1E2A5E" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {platformPopularity.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={platformPopularity} layout="vertical">
                ...
                <Tooltip />
                <Bar dataKey="count" fill="#00C2D1" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
...
function QuickAction({ icon: Icon, label, desc, onClick }: any) {
  return (
    ...
    >
      <Icon className="h-6 w-6 text-[#1E2A5E] mb-2" />
